...
| `js/09-sync.js` | **Google Drive sync** (optional; needs `GDRIVE_CLIENT_ID`). Offline-first newest-wins merge with deletion tombstones. `SYNC` state; `DriveRemote` (drive.file REST adapter) + `MockRemote` (tests, via `window.__mockDriveStore`); redirect OAuth (`syncAuthRedirect`/`syncParseRedirect`, iOS-PWA safe); `runSync(manual)` = pull manifest → merge records (per-id `updatedAt`, tombstones block resurrection) → download remote-won snippets+blobs → upload missing blobs → write merged manifest → prune remote orphans; audio stored as `au_<id>_r<rev>` (immutable per `audioRev`), recordings as `rec_<id>`; triggers: debounced `syncQueue` on writes, visibilitychange, manual "Sync now". Settings UI via `renderSyncSection`; status dot `#syncDot`. Setup steps in `SYNC_SETUP.md`. |
| `sw.js` | Precaches ALL files in CORE (**update list when adding files**) using `Request(url,{cache:'reload'})` — **never remove that**, or a version bump can precache stale HTTP-cached assets and freeze the old app in. Runtime cache-first otherwise. **Bump `VERSION` on every release**. |
...
pitch-v2/print-v2 · 8 loop deep-test · 9 Drive-sync merge engine (mock adapter). ~267 checks total; keep them green.
